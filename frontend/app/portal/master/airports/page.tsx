...
import { listAirports, saveAirport } from "@/services/portalMaster";
...
export default function PortalAirportsPage() {
  const fetchRows = useCallback(async (search: string) => {
    const res = await listAirports({ limit: 200, search: search.trim() || undefined });
    return res.data;
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formAlerts.setSubmitError(e instanceof Error ? e.message : "Failed to save airport");
    }
  };
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Master data — airports used on routes and crew bases</p>
        </div>
        <PortalAddButton onClick={openCreate}>New airport</PortalAddButton>
      </div>
              ...
              {rows.length === 0 ? (
                ...
                rows.map((r) => (
                  ...
                  >
                    ...
                    <TableCell>{String(r.airport_name)}</TableCell>
                    <TableCell>{String(r.city)}</TableCell>
                    <TableCell>{String(r.country)}</TableCell>
                    <TableCell>{r.is_active ? "Yes" : "No"}</TableCell>
                    <TableCell className="text-right">
                      <ListRowActions doctype="Airport" docName={String(r.name)}>
                        <DropdownMenu>
      ...
      >
        <FormGrid>
          <FormField label="Airport name" required fullWidth>
            <Input
          ...
          <FormField label="Active" fullWidth>
            <div className="flex h-9 items-center gap-2">
              ...
              />
              <span className="text-sm text-muted-foreground">Available for routes</span>
            </div>
          </FormField>
        </FormGrid>
      </BilanFormDialog>
